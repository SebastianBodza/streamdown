...
<TypeTable
  type={{
    ...
    parseIncompleteMarkdown: {
      description: 'Enable remend preprocessor for unterminated Markdown blocks',
      type: 'boolean',
      default: 'true',
    },
    remend: {
...
<TypeTable
  type={{
    ...
    link: {
      description: 'Custom link component and internal/external link detection',
      type: 'LinkOptions',
    },
    controls: {
...
The `link` prop accepts an object with the following properties:

<TypeTable
  type={{
    component: {
      description: 'Custom React component for rendering links',
      type: 'React.ComponentType<LinkComponentProps>',
    },
    allowedPrefixes: {
      description: 'URL prefixes that should be considered internal links',
      type: 'string[]',
    },
  }}
/>

See the [Custom Link Component](/docs/custom-links) documentation for detailed usage examples.
